...
typedef uint32_t hri_rtcmode2_intenset_reg_t;
...
public:
  ...
  enum Alarm_Match: uint8_t // Should we have this enum or just use the identifiers from /component/rtc.h ?
  {
    ...
    MATCH_SS_REPEAT    = 0x7ul                                // Repeat each second
  };
...
private:
  ...
  void RTCMode2_configCTRLA(void);
  void RTCMode2_configALARM0(void);
  void RTCMode2_waitforSync(hri_rtcmode2_syncbusy_reg_t reg);
  bool RTCMode2_isSyncing(hri_rtcmode2_syncbusy_reg_t reg);
  void RTCMode2_disable();
  void RTCMode2_enable();
  void RTCMode2_reset();
  void RTCMode2_resetRemove();
};
